...
export default function GetFactForm({ cityName, cityDescription, imageUrl, wikiUrl }: Props) {
    ...
    async function handleGetFact(event: React.MouseEvent<HTMLButtonElement>): Promise<void> {
        ...
        try {
            const response = await axios.post("http://localhost:8080/api/insight/city", wikiUrl, {
                headers: {
  ...
  return (
    <div className="grid grid-cols-4 gap-1 p-8 pt-4">
        ...
        <button 
            className="h-[50px] col-span-1 bg-green hover:bg-light-green text-gray flex items-center justify-center rounded-br-lg transform active:scale-y-75 transition-transform"
